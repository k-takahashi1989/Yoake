...
|---|---|---|---|
| 睡眠記録（手動） | ✅ | 🚧 | ビルド未検証 |
| Apple Health 連携 | ✅ HC | ❌ | `react-native-health` が RN 0.84 (New Architecture) と非互換のため除去。`appleHealth.ts` はスタブ（常に null/false）。互換ライブラリ確定後に再実装予定。 |
...
### Apple Health 実装状況（スタブ中）

`react-native-health` が RN 0.84 New Architecture で `setBridge:` セレクタの削除により Xcode ビルドが失敗するため、パッケージを除去してスタブ実装に差し替え済み。
...
src/services/
  ...
  appleHealth.ts     ← iOS スタブ（常に null/false）
